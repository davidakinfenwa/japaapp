...
class CountryPredictionCubit extends Cubit<BlocState<Failure<ExceptionMessage>, CountryPredictionModel>> {
  final JoruneyRepository _repository;
  final JourneySnapshotCache _snapshotCache;

  CountryPredictionCubit({
  ...
  Future<void> countryPrediction() async {
    emit(const BlocState<Failure<ExceptionMessage>, CountryPredictionModel>.loading());

    final _signupEither = await _repository.countryPrediction();
    ...
    final _state = _signupEither.fold(
      ...
      (predictionInfoModel) {
        // cache snapshot
          _snapshotCache.countryPredictionModel = predictionInfoModel;
